...
function render(){
  ...
  Object.entries(groups).forEach(([key,label])=>{
    ...
    items.filter(it=>it.group===key).forEach(it=>{
      ...
      d.innerHTML=`<div class="row"><input class="check" type="checkbox" ${st.done?'checked':''}><div style="flex:1"><div class="title">${esc(it.title)}</div><div class="meta"><span class="badge ${badgeClass(it.tag)}">${esc(it.tag)}</span>${esc(it.desc)}</div><div class="stamp">${stamp}</div><textarea class="note" rows="1" placeholder="備註／申請編號／預約時間…">${esc(st.note||'')}</textarea></div></div>`;
